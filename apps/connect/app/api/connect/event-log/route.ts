...
import { writeConnectEvent } from "../../../../src/lib/event-log";
...
export async function POST(request: Request) {
  ...
  await writeConnectEvent({
    companyId: account.companyId,
    platform,
    eventCode: String(body.eventCode ?? "screen_view"),
    module: String(body.module ?? "dropx_one"),
    action: String(body.action ?? "view"),
    actorType: profileType,
    actorAccountId: account.id,
    actorLabel: account.name,
    actorIdentifier: `+${session.country_code} ${String(session.mobile_number).replace(new RegExp(`^${session.country_code}`), "")}`,
