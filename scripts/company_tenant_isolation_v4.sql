...
begin
  ...
  if to_regclass('public.location_models') is not null then
  ...
  if to_regclass('public.user_roles') is not null then
    alter table public.user_roles
      add column if not exists company_id uuid references public.companies(id) on delete cascade;
    update public.user_roles set company_id = master_company where company_id is null;
    alter table public.user_roles drop constraint if exists user_roles_code_key;
    drop index if exists public.user_roles_code_key;
    create unique index if not exists user_roles_company_code_uidx
    ...
    where not exists (
      ...
      where user_roles.company_id = companies.id
        and user_roles.code = 'OWNER'
    );

    update public.profiles
    set role_id = owner_roles.id,
        updated_at = now()
    from public.user_roles owner_roles
    left join public.companies company_row on company_row.id = owner_roles.company_id
    where profiles.company_id = owner_roles.company_id
      and owner_roles.code = 'OWNER'
      and (
        profiles.is_master_owner = true
        or profiles.invite_method in ('Company Admin', 'Platform Admin')
        or lower(profiles.email) = lower(company_row.admin_email)
      );
  end if;
  ...
  if to_regclass('public.whatsapp_notification_configs') is not null then
    ...
    alter table public.whatsapp_notification_configs drop constraint if exists whatsapp_notification_configs_template_company_fkey;
  ...
  if to_regclass('public.secrets') is not null then
    alter table public.secrets
      add column if not exists company_id uuid references public.companies(id) on delete cascade;
    update public.secrets set company_id = master_company where company_id is null;
    drop index if exists public.secrets_name_idx;
    create unique index if not exists secrets_company_name_idx
      on public.secrets(company_id, name);
    delete from public.secrets where company_id <> master_company;
  end if;
