create table if not exists public.dropx_id_generation_settings (
  ...
  scope_type text not null default 'category' check (scope_type in ('company', 'category', 'model', 'location', 'designation', 'multi_designation')),
...
alter table public.dropx_id_generation_settings
  ...
  check (scope_type in ('company', 'category', 'model', 'location', 'designation', 'multi_designation'));
...
begin
  ...
  if (select array_agg(key order by key) from jsonb_object_keys(selected_setting.configs) key)
     is distinct from (select array_agg(key order by key) from jsonb_object_keys(p_configs) key) then
    raise exception 'Series cannot be added or removed after ID generation has started.';
  end if;
  for selected_key in select jsonb_object_keys(selected_setting.configs) loop
    if (selected_setting.configs -> selected_key) - 'designation_ids' is distinct from (p_configs -> selected_key) - 'designation_ids' then
  ...
  if exists (
    with proposed as (
      select distinct designation_id from jsonb_each(p_configs) series
      cross join lateral jsonb_array_elements_text(coalesce(series.value -> 'designation_ids', '[]'::jsonb)) as mapped(designation_id)
    ), existing as (
      select distinct designation_id from jsonb_each(selected_setting.configs) series
      cross join lateral jsonb_array_elements_text(coalesce(series.value -> 'designation_ids', '[]'::jsonb)) as mapped(designation_id)
    )
    select 1 from proposed left join existing using (designation_id)
    left join public.designations designations on designations.id::text = proposed.designation_id
      and designations.company_id = p_company_id and designations.is_active = true
    where existing.designation_id is null and designations.id is null
  ) then raise exception 'One or more new designations are not active for this company.'; end if;
  select jsonb_object_agg(series.key, jsonb_set(series.value, '{designation_ids}', coalesce(p_configs -> series.key -> 'designation_ids', '[]'::jsonb), true))
    into merged_configs from jsonb_each(selected_setting.configs) series;
  update public.dropx_id_generation_settings set configs = merged_configs, updated_at = now() where id = selected_setting.id;
end;
$$;
