    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      body: JSON.stringify({
        messages: [
          {
            role: "system",
            content: "You are DropX Capacity AI. Return a JSON object named actions mapping every supplied stationCode to exactly one concise operational action of no more than 18 words. Use only supplied facts. Prioritize hiring shortage, ad hoc dependency, high SPR, surplus reallocation, missing ground update, or stable capacity. Never invent a cause."
          },
