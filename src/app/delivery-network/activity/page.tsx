...
type AttendanceRow = { id: string; account_id: string | null; field_executive_id: string | null; punch_date: string; worker_status: string | null };
...
async function loadAttendanceRows(companyId: string, from: string, to: string, locationScopeIds: string[] | null) {
  ...
  for (let start = 0; start < 50000; start += pageSize) {
    let query = supabaseAdmin.from("attendance_punches")
      .select("id, account_id, field_executive_id, punch_date, worker_status")
...
export default async function WorkforceActivityPage({ searchParams }: { searchParams?: { from?: string; to?: string } }) {
  ...
  if (supabaseAdmin) {
    ...
    const [attendanceResult, workforceResult] = await Promise.all([
    ...
    attendanceError = attendanceResult.error ?? workforceResult.error?.message ?? (attendanceResult.truncated ? "Attendance source exceeded 50,000 rows. Narrow the date range for complete person-day totals." : null);
  }
  const snapshot = await snapshotPromise;

  const presentRows = attendanceRows.filter((row) => Boolean(row.account_id ?? row.field_executive_id) && !["A", "ABSENT", "INACTIVE"].includes(String(row.worker_status ?? "").toUpperCase()));
  const attendedPeople = new Set(presentRows.map((row) => row.account_id ?? row.field_executive_id)).size;
  const attendedPersonDays = new Set(presentRows.map((row) => `${row.account_id ?? row.field_executive_id}:${row.punch_date}`)).size;
  ...
  return (
    <AppShell active="Attendance & Activity" pageCode="workforce_activity">
      ...
      <section className="wf-finance-kpis">
        ...
        <article><span><Fingerprint size={18} /></span><small>Shipment-active IDs</small><strong>{format(shipmentWorkers)}</strong><em>{format(snapshot.exceptions.length)} mapping/rate gaps</em></article>
        ...
        <article><span><Gauge size={18} /></span><small>Average productivity</small><strong>{format(average, 1)}</strong><em>Deliveries per active ID</em></article>
      </section>

      <section className="wf-finance-panel">
        <header>
          <div><span>Daily production ledger</span><h2>Latest associate activity</h2><p>Every row traces back to the imported provider employee ID and business date.</p></div>
