...
export type AmazonStationSettings={station_id:string;service_area_code:string;service_type:string;supervisor_alias:string;contract_type:string;associate_email_pattern:string|null;invitation_enabled:boolean;version:number;updated_at:string};
export default async function AmazonSettings({searchParams:params={}}:{searchParams?:{station?:string;notice?:string;error?:string}}) {
  ...
  const [stationsResult,settingsResult]=await Promise.all([readAllRows(query),readAllRows(supabaseAdmin.from("workforce_amazon_station_settings").select("station_id,service_area_code,service_type,supervisor_alias,contract_type,associate_email_pattern,invitation_enabled,version,updated_at").eq("company_id",company).order("station_id"))]);
  ...
  return <AppShell active="Amazon Station Defaults" pageCode="executive_id_onboarding"><div className={styles.desk}>
    <PageHead eyebrow="Workforce configuration" title="Amazon activation master" subtitle="Configure each station’s Amazon service area, associate email rule and invitation readiness." action={<Link href="/delivery-network/id-onboarding" className="button secondary compact">Open activation desk</Link>}/>
      ...
      {station ? <div className={styles.detail}><section className={styles.card}><header><div><h2>{station.station_code}</h2><p>Verify against Amazon Setup → Associates → Onboarding → Associate Settings. Defaults are a handoff reference, not proof the external setup is complete.</p></div></header>
        <form className={styles.plan} action={saveAmazonStation} key={station.id}>
          ...
          <fieldset disabled={!canEdit || Boolean(stationsResult.error || settingsResult.error)} style={{border:0,padding:0,margin:0}}><div className={styles.fields}>
            ...
            <label>Associate email pattern<input name="associate_email_pattern" required maxLength={254} defaultValue={current?.associate_email_pattern ?? ""} placeholder="{first_name}.{station_code}@yourdomain.com"/><small>Must include {'{station_code}'} and either {'{first_name}'} or {'{full_name}'}.</small></label>
            <label className="checkbox-row"><input name="invitation_enabled" type="checkbox" defaultChecked={current?.invitation_enabled??false}/> Enable Create Amazon ID for this station</label>
          </div><p className={styles.hint}>No mailbox passwords, OTPs or Amazon credentials belong here. The system validates the entered address against this pattern before it queues an invitation.</p><SubmitButton pendingText="Saving station defaults">Save station defaults</SubmitButton></fieldset>
        </form></section><section className={styles.card}><header><h3>Change history · IST</h3></header><ol className={styles.history}>{(history?.data ?? []).map(row=><li key={row.id}><strong>{row.actor_name}</strong><span>{new Date(row.created_at).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"})}</span><p>{row.details?.after?.service_area_code} · {row.details?.after?.service_type} · {row.details?.after?.associate_email_pattern || "No email rule"} · {row.details?.after?.invitation_enabled ? "Enabled":"Paused"}</p></li>)}{!history?.data?.length ? <li>{history?.error?.message || "No settings saved yet."}</li>:null}</ol></section></div>:<p>No permitted stations found.</p>}
    </div>
  </div></AppShell>;
}
