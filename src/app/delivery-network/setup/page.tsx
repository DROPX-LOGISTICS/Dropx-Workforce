...
export default async function WorkforceSetup({searchParams:params={}}:{searchParams?:{q?:string;view?:string}}){
 ...
 const rows=(data?.stations??[]).map(station=>{
  ...
  return {...station,people:people.length,calendar,amazon,missing:Number(!calendar)+Number(!amazon)};
 });
 ...
 return <AppShell active="Setup Checklist" pageCode="people_review"><div className={styles.desk}>
  <PageHead eyebrow="Owner workspace · Configuration" title="Workforce setup checklist" subtitle="Station settings, Amazon activation and the route from approval to Finance."/>
  {error?<p role="alert" className={styles.error}>{error}</p>:<>
   <section className={styles.stats}><article><small>Stations in scope</small><strong>{rows.length}</strong></article><article><small>With current profiles</small><strong>{rows.filter(row=>row.people>0).length}</strong></article><article><small>Required masters present</small><strong>{rows.filter(row=>!row.missing).length}</strong><small>Still review individual rates and payment stages</small></article></section>
   <section className={styles.card}><header><div><h2>Station masters</h2><p>Payroll calendars control settlement periods. Amazon defaults control email naming and invitation availability for Amazon stations.</p></div></header>
...
<div className={styles.table}><table><thead><tr><th>Station</th><th>Current profiles</th><th>Payroll calendar</th><th>Amazon activation</th></tr></thead><tbody>{visible.map(row=><tr key={row.id}><th>{row.station_code}</th><td>{row.people}</td><td><Link href={`/delivery-network/payroll-calendars?station=${row.id}`}>{row.calendar?'Available today':'Manual dates / configure'}</Link></td><td><Link href={`/delivery-network/amazon-onboarding-settings?station=${row.id}`}>{row.amazon?'Saved · verify rules':'Configure Amazon master'}</Link></td></tr>)}{!visible.length?<tr><td colSpan={4}>No stations match this view.</td></tr>:null}</tbody></table></div>
   </section>
  ...
  <section className={styles.card}><header><h2>Operating desks</h2></header><div className={styles.links}><Link href="/delivery-network/id-onboarding?view=pending">Amazon ID & activation</Link><Link href="/delivery-network/rate-mapping">Provider ID & effective rates</Link><Link href="/delivery-network/earnings">Earnings & exceptions</Link><Link href="/delivery-network/payment-holds">Payment holds</Link><Link href="/delivery-network/adjustments">Claim review</Link><Link href="/delivery-network/lifecycle?tab=exits">Exits & Finance reconciliation</Link></div></section>
 </div></AppShell>;
}
