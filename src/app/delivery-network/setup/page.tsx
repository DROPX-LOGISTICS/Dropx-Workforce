...
<div className={styles.table}><table><thead><tr><th>Station</th><th>Current profiles</th><th>Training terms</th><th>Payroll calendar</th><th>Amazon defaults</th></tr></thead><tbody>{visible.map(row=><tr key={row.id}><th>{row.station_code}</th><td>{row.people}</td><td><Link href={`/delivery-network/training-policies?station=${row.id}`}>{row.policy?'Available today':'Configure if training'}</Link></td><td><Link href={`/delivery-network/payroll-calendars?station=${row.id}`}>{row.calendar?'Available today':'Manual dates / configure'}</Link></td><td><Link href={`/delivery-network/amazon-onboarding-settings?station=${row.id}`}>{row.amazon?'Saved · verify with Amazon':'Configure if Amazon'}</Link></td></tr>)}{!visible.length?<tr><td colSpan={5}>No stations match this view.</td></tr>:null}</tbody></table></div>
   </section>
