...
export default async function CapacityMasterPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity Master" pageCode="cod_master"><div className="ops-command-center">
    ...
    <section className="panel capacity-bulk-panel"><div className="panel-head"><div><h2>Bulk setup</h2><p className="subtle">Initialize or replace the planning assumptions for Amazon EDSP and XPT stations. Each station remains editable afterward.</p></div><form action={bulkInitializeCapacityRules} className="capacity-bulk-form"><label>Target SPR<input name="target_spr" type="number" min="1" step=".1" defaultValue="60" required/></label><label>Maximum safe SPR<input name="max_safe_spr" type="number" min="1" step=".1" defaultValue="70" required/></label><label>Buffer %<input name="buffer_percent" type="number" min="0" step=".5" defaultValue="10" required/></label><label>Recent days<input name="recent_days" type="number" min="1" max="31" defaultValue="6" required/></label><SubmitButton confirmMessage="Apply these assumptions to every Amazon EDSP and XPT station?" confirmTitle="Initialize Amazon capacity rules" confirmSubmitText="Apply to all" disabled={!permission.canEdit}>Apply to all stations</SubmitButton></form></div></section>
