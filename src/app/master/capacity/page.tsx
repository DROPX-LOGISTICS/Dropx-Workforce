...
export default async function CapacityMasterPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity Master" pageCode="cod_master"><div className="ops-command-center">
    ...
    <section className="panel capacity-bulk-panel"><div className="panel-head"><div><h2>Shipment size classification</h2><p className="subtle">A shipment is volumetric when any dimension exceeds the configured shoe-box limit, or when actual weight exceeds the weight limit. The remaining classified shipments are small.</p></div><form action={upsertShipmentSizeRule} className="capacity-bulk-form"><label>Length cm<input name="max_length_cm" type="number" min="1" step=".1" defaultValue={sizeResult.rule?.maxLengthCm ?? 35} required/></label><label>Width cm<input name="max_width_cm" type="number" min="1" step=".1" defaultValue={sizeResult.rule?.maxWidthCm ?? 22} required/></label><label>Height cm<input name="max_height_cm" type="number" min="1" step=".1" defaultValue={sizeResult.rule?.maxHeightCm ?? 13} required/></label><label>Weight kg<input name="max_weight_kg" type="number" min=".1" step=".1" defaultValue={sizeResult.rule?.maxWeightKg ?? 5} required/></label><SubmitButton disabled={!permission.canEdit}>Save size rule</SubmitButton></form></div></section>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Planning rules</h2><p className="subtle">Target SPR drives required headcount. Maximum safe SPR flags overload. Buffer protects peak and attendance variation.</p></div></div>
      <div className="table-wrap"><table className="capacity-master-table"><thead><tr><th>Station</th><th>Target SPR</th><th>Maximum safe SPR</th><th>Headcount buffer</th><th>Recent baseline</th><th>Status</th><th>Actions</th></tr></thead><tbody>
        {capacityLocations.map((location) => {
          ...
          return <tr key={location.id}><td><strong>{location.station_code}</strong><small>{location.station_name || location.city || "—"}</small></td><td colSpan={6}><div className="capacity-master-row"><form action={upsertCapacityRule}><input type="hidden" name="station_code" value={location.station_code}/><input aria-label={`${location.station_code} target SPR`} name="target_spr" type="number" min="1" step=".1" defaultValue={rule?.targetSpr ?? ""} placeholder="e.g. 60" required/><input aria-label={`${location.station_code} maximum safe SPR`} name="max_safe_spr" type="number" min="1" step=".1" defaultValue={rule?.maxSafeSpr ?? ""} placeholder="e.g. 70" required/><input aria-label={`${location.station_code} buffer percent`} name="buffer_percent" type="number" min="0" step=".5" defaultValue={rule?.bufferPercent ?? ""} placeholder="e.g. 10" required/><input aria-label={`${location.station_code} recent days`} name="recent_days" type="number" min="1" max="31" defaultValue={rule?.recentDays ?? 6} required/><span className={`status-pill ${rule ? "good" : "warn"}`}>{rule ? "Configured" : "Not configured"}</span><SubmitButton disabled={!permission.canEdit}>{rule ? "Save" : "Configure"}</SubmitButton></form>{rule?.id ? <form action={removeCapacityRule}><input type="hidden" name="id" value={rule.id}/><SubmitButton className="button danger compact" confirmMessage={`Delete capacity assumptions for ${location.station_code}?`} confirmSubmitText="Delete rule" disabled={!permission.canEdit}>Delete</SubmitButton></form> : null}</div></td></tr>;
        })}
      </tbody></table></div>
    </section>
  </div></AppShell>;
