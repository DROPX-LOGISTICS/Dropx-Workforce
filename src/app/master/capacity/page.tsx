...
export default async function CapacityMasterPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity Master" pageCode="cod_master"><div className="ops-command-center">
    ...
    <section className="panel capacity-bulk-panel"><div className="panel-head"><div><h2>Hiring decision defaults</h2><p className="subtle">Apply the same baseline, ground-data gate and alert thresholds to every permitted station.</p></div></div><form action={bulkInitializeCapacityRules} className="capacity-bulk-form capacity-bulk-planning-form"><label>Target SPR<input name="target_spr" type="number" min="1" step=".1" defaultValue="60" required/></label><label>Safe SPR<input name="max_safe_spr" type="number" min="1" step=".1" defaultValue="70" required/></label><label>Buffer %<input name="buffer_percent" type="number" min="0" step=".5" defaultValue="10" required/></label><label>Baseline days<input name="recent_days" type="number" min="14" max="31" defaultValue={CAPACITY_PLANNING_DEFAULTS.baselineDays} required/></label><label>Min matched days<input name="minimum_matched_days" type="number" min="1" max="31" defaultValue={CAPACITY_PLANNING_DEFAULTS.minimumMatchedDays} required/></label><label>DA drop alert %<input name="associate_drop_percent" type="number" min="1" max="100" defaultValue={CAPACITY_PLANNING_DEFAULTS.associateDropPercent} required/></label><label>Volume spike %<input name="volume_spike_percent" type="number" min="1" max="300" defaultValue={CAPACITY_PLANNING_DEFAULTS.volumeSpikePercent} required/></label><SubmitButton confirmMessage="Apply these assumptions to every Amazon EDSP and XPT station?" confirmTitle="Initialize Amazon capacity rules" confirmSubmitText="Apply to all" disabled={!permission.canEdit}>Apply to all stations</SubmitButton></form></section>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Station planning rules</h2><p className="subtle">Every input is aligned to its header. Baseline and matched-day gates control permanent-hiring decisions; alert percentages control dashboard notifications.</p></div></div>
      <div className="table-wrap"><table className="capacity-master-table capacity-master-planning-table"><thead><tr><th>Station</th><th>Target SPR</th><th>Safe SPR</th><th>Buffer %</th><th>Baseline days</th><th>Min matched</th><th>DA drop %</th><th>Volume spike %</th><th>Status</th><th>Actions</th></tr></thead><tbody>
        {capacityLocations.map((location) => {
          ...
          return <tr key={location.id}>
            ...
            <td><input aria-label={`${location.station_code} minimum matched days`} form={formId} name="minimum_matched_days" type="number" min="1" max="31" defaultValue={settings.minimumMatchedDays} required/></td>
