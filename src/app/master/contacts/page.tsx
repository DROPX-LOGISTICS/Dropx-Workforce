...
type Contact = { id: string; contact_no: string | null; email: string | null; bank_account_no: string; ifsc: string; account_holder_name: string; verified_at: string | null };
...
export default async function ContactsPage({ searchParams }: { searchParams?: { edit?: string } }) {
  ...
  const result = await supabaseAdmin.from("payment_contacts").select("id, contact_no, email, bank_account_no, ifsc, account_holder_name, verified_at").eq("company_id", companyId).order("account_holder_name");
  if (result.error) throw new Error(`${result.error.message} Run scripts/payment_contacts_upi_resubmission_v2.sql in Supabase SQL Editor.`);
  ...
  return <AppShell active="Contacts" pageCode="master_contacts">
    <PageHead eyebrow="Master Data" title="Contacts" subtitle="Reuse verified beneficiary accounts without repeating paid bank verification." />
    ...
    {permission.canView ? <section className="panel"><div className="panel-head"><div><h2>Contact list</h2><p className="subtle">{contacts.length} records</p></div></div><div className="table-wrap"><table><thead><tr><th>Account holder</th><th>Bank account no</th><th>IFSC</th><th>Contact no</th><th>Email</th><th>Source</th>{permission.canEdit ? <th>Action</th> : null}</tr></thead><tbody>{contacts.length ? contacts.map((contact) => <tr key={contact.id}><td><strong>{contact.account_holder_name}</strong></td><td>{contact.bank_account_no}</td><td>{contact.ifsc}</td><td>{contact.contact_no || "-"}</td><td>{contact.email || "-"}</td><td>{contact.verified_at ? "Verified" : "Manual"}</td>{permission.canEdit ? <td><PendingLink className="button secondary compact" href={`/master/contacts?edit=${contact.id}`} scroll={false}>Edit</PendingLink></td> : null}</tr>) : <tr><td className="empty-cell" colSpan={permission.canEdit ? 7 : 6}>No contacts added yet.</td></tr>}</tbody></table></div></section> : null}
