...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="performance-summary-grid"><article><span>Average road IDs</span><strong>{fmt(averageIds, 1)}</strong><small>Daily active IDs</small></article><article><span>Average workload</span><strong>{fmt(averageVolume)}</strong><small>Amazon + SWA + C-return</small></article><article><span>Average inbound</span><strong>{averageInbound ? fmt(averageInbound) : "—"}</strong><small>Expected packages at station</small></article><article><span>Average SPR</span><strong>{fmt(averageSpr, 1)}</strong><small>Workload ÷ road-active IDs</small></article><article><span>Required IDs</span><strong>{requiredIds ?? "—"}</strong><small>{targetSpr ? `SPR ${fmt(targetSpr, 1)} + ${fmt(buffer)}% buffer` : "Configure master"}</small></article></section>
