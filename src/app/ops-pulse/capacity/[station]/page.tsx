...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="panel"><div className="panel-head"><div><h2>Day-level capacity</h2><p className="subtle">Road IDs blend Daily Shipment Count and delivered detail. Inbound is shown separately and never substituted for delivered productivity.</p></div></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Date</th><th>Active IDs</th><th>Low-volume IDs</th><th>Actual regular</th><th>Ad hoc</th><th>Delivered</th><th>Inbound</th><th>SPR</th><th>Source</th><th>Position</th></tr></thead><tbody>
      {daily.map((day) => { const review = reviewMap.get(day.date); const required = targetSpr ? Math.ceil(day.delivered / targetSpr * (1 + buffer / 100)) : null; const reliable = review ? num(review.regularPresent) : day.ids; const gap = required == null ? null : required - reliable; return <tr key={day.date}><td><a href={`/ops-pulse/capacity/${stationCode}?from=${start}&to=${end}&review_date=${day.date}`}>{day.date.split("-").reverse().join("/")}</a></td><td><strong>{day.ids}</strong></td><td><span className={day.lowVolumeIds ? "metric-bad-text" : ""}>{day.lowVolumeIds || "—"}</span></td><td>{review ? num(review.regularPresent) : "—"}</td><td>{review ? num(review.adHocPresent) : "—"}</td><td>{fmt(day.delivered)}</td><td>{day.inbound ? fmt(day.inbound) : "—"}</td><td><strong>{fmt(day.spr, 1)}</strong></td><td><small>{day.source}</small></td><td><span className={`capacity-decision ${gap == null ? "unconfigured" : gap > 0 ? "hire" : gap < -1 ? "surplus" : "balanced"}`}>{!review ? "Review actuals" : gap == null ? "Configure" : gap > 0 ? `Reliable short ${gap}` : gap < -1 ? `Reliable above ${Math.abs(gap)}` : "Covered"}</span></td></tr>; })}
      {!daily.length ? <tr><td className="empty-cell" colSpan={10}>No capacity source data in this range.</td></tr> : null}
