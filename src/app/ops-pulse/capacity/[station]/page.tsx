...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  const averageRegular = daily.length ? daily.reduce((sum, day) => sum + day.regular, 0) / daily.length : 0;
  const averageAdHoc = daily.length ? daily.reduce((sum, day) => sum + day.adHoc, 0) / daily.length : 0;
  const adHocDays = daily.filter((day) => day.adHoc > 0).length;
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="performance-summary-grid capacity-actual-summary"><article><span>Regular IDs</span><strong>{daily.length ? fmt(averageRegular, 1) : "—"}</strong><small>Average total IDs less approved ad-hoc</small></article><article><span>Ad-hoc IDs</span><strong>{daily.length ? fmt(averageAdHoc, 1) : "—"}</strong><small>Average from approved payments</small></article><article><span>Ad-hoc days</span><strong>{adHocDays}/{daily.length}</strong><small>Days with approved temporary DAs</small></article><article><span>Approved requests</span><strong>{approvedRequests}</strong><small>Included in selected period</small></article></section>
    <div className="capacity-action-line"><strong>Action</strong><span>{action}</span></div>
    <section className="panel"><div className="panel-head"><div><h2>Day-level capacity</h2><p className="subtle">Amazon road IDs split automatically using final-approved ad-hoc DA payment requests.</p></div></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Date</th><th>Total IDs</th><th>Regular IDs</th><th>Ad-hoc IDs</th><th>Approved requests</th><th>Workload</th><th>Inbound</th><th>SPR</th></tr></thead><tbody>
      {daily.map((day) => <tr key={day.date}><td>{day.date.split("-").reverse().join("/")}</td><td><strong>{day.ids}</strong></td><td>{day.regular}</td><td><strong className={day.adHoc ? "metric-warn-text" : ""}>{day.adHoc}</strong></td><td>{day.paymentRequests || "—"}</td><td>{fmt(day.delivered)}</td><td>{day.inbound ? fmt(day.inbound) : "—"}</td><td><strong>{fmt(day.spr, 1)}</strong></td></tr>)}
