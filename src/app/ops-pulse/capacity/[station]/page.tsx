...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    <PageHead eyebrow="Station Capacity" title={`${stationCode} · ${location.station_name || location.city || stationCode}`} subtitle="Blended road-ID headcount, delivered and inbound demand, and allocation productivity." />
    ...
    <section className="performance-summary-grid"><article><span>Average road IDs</span><strong>{fmt(averageIds, 1)}</strong><small>Daily count + delivered detail</small></article><article><span>Average delivered</span><strong>{fmt(averageVolume)}</strong><small>Preferred delivered source</small></article><article><span>Average inbound</span><strong>{averageInbound ? fmt(averageInbound) : "—"}</strong><small>Expected packages at station</small></article><article><span>Average SPR</span><strong>{fmt(averageSpr, 1)}</strong><small>Delivered ÷ road-active IDs</small></article><article><span>Required IDs</span><strong>{requiredIds ?? "—"}</strong><small>{targetSpr ? `SPR ${fmt(targetSpr, 1)} + ${fmt(buffer)}% buffer` : "Configure master"}</small></article></section>
    <section className="performance-summary-grid capacity-actual-summary"><article><span>Updated days</span><strong>{reviewedDays}/{daily.length}</strong><small>Ground classifications saved</small></article><article><span>Regular IDs</span><strong>{reviews.length ? fmt(averageActualRegular, 1) : "—"}</strong><small>Average bike + van</small></article><article><span>Ad hoc IDs</span><strong>{reviews.length ? fmt(averageAdHoc, 1) : "—"}</strong><small>Average bike + van</small></article><article><span>Vehicle mix</span><strong>{reviews.length ? `${fmt(averageBike, 1)} / ${fmt(averageVan, 1)}` : "—"}</strong><small>Average bike / van IDs</small></article></section>
    <div className="capacity-action-line"><strong>Action</strong><span>{action}</span></div>
    <section className="panel capacity-ground-entry-link"><div className="panel-head"><div><h2>Daily ground update</h2><p className="subtle">Assigned packages and regular/ad hoc bike/van classification are maintained in one shared workspace.</p></div><a className="button compact" href={`/ops-pulse/capacity/daily?date=${dates.at(-1) ?? end}&stations=${stationCode}`}>Update {stationCode}</a></div></section>
    <section className="panel"><div className="panel-head"><div><h2>Day-level capacity</h2><p className="subtle">System demand and the saved ground classification in one timeline.</p></div></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Date</th><th>IDs used</th><th>Regular bike</th><th>Regular van</th><th>Ad hoc bike</th><th>Ad hoc van</th><th>Assigned</th><th>Delivered</th><th>Inbound</th><th>SPR</th><th>Status</th></tr></thead><tbody>
      {daily.map((day) => { const review = reviewMap.get(day.date); const classified = review ? num(review.classifiedIds) : null; const difference = classified == null ? null : day.ids - classified; return <tr key={day.date}><td><a href={`/ops-pulse/capacity/daily?date=${day.date}&stations=${stationCode}`}>{day.date.split("-").reverse().join("/")}</a></td><td><strong>{day.ids}</strong></td><td>{review ? num(review.regularBike) : "—"}</td><td>{review ? num(review.regularVan) : "—"}</td><td>{review ? num(review.adHocBike) : "—"}</td><td>{review ? num(review.adHocVan) : "—"}</td><td>{review ? fmt(num(review.assignedPackages)) : "—"}</td><td>{fmt(day.delivered)}</td><td>{day.inbound ? fmt(day.inbound) : "—"}</td><td><strong>{fmt(day.spr, 1)}</strong></td><td><span className={`capacity-entry-status ${difference == null ? "pending" : difference === 0 ? "matched" : "different"}`}>{difference == null ? "Not updated" : difference === 0 ? "Matched" : `Difference ${difference > 0 ? "+" : ""}${difference}`}</span></td></tr>; })}
      {!daily.length ? <tr><td className="empty-cell" colSpan={11}>No capacity source data in this range.</td></tr> : null}
