...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  const allocations = allIds.map((id) => {
    ...
    return { id, name: idRows.find((row) => row.associate_name)?.associate_name || "Unmapped name", days: workedDates.length, delivered, volumetric, small, average: workedDates.length ? delivered / workedDates.length : 0 };
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="capacity-station-columns"><div className="panel"><div className="panel-head"><div><h2>Associate allocation</h2><p className="subtle">Every shipment-active ID attributed to this station. XPT activity remains with its XPT code.</p></div><a className="button secondary compact" href={`/capacity/associates?station=${stationCode}&from=${start}&to=${end}&preset=custom`}>Open full SPR view</a></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Associate</th><th>Days</th><th>Delivered</th><th>Average/day</th><th>Volumetric</th><th>Small</th><th>Workload position</th></tr></thead><tbody>{allocations.map((row) => <tr key={row.id}><td><a className="capacity-station-link" href={`/capacity/associates/${encodeURIComponent(row.id)}?station=${stationCode}&from=${start}&to=${end}`}><strong>{row.name}</strong><small>{row.id}</small></a></td><td>{row.days}</td><td>{fmt(row.delivered)}</td><td><strong className={row.average > (rule?.maxSafeSpr ?? 70) ? "metric-bad-text" : ""}>{fmt(row.average, 1)}</strong></td><td>{fmt(row.volumetric)}<small>{row.delivered ? `${fmt(row.volumetric / row.delivered * 100, 1)}%` : "—"}</small></td><td>{fmt(row.small)}<small>{row.delivered ? `${fmt(row.small / row.delivered * 100, 1)}%` : "—"}</small></td><td><span className={`capacity-decision ${row.average > (rule?.maxSafeSpr ?? 70) ? "risk" : row.average < (rule?.targetSpr ?? 60) ? "unconfigured" : "balanced"}`}>{row.average > (rule?.maxSafeSpr ?? 70) ? "Above safe" : row.average < (rule?.targetSpr ?? 60) ? "Below target" : "Target range"}</span></td></tr>)}</tbody></table></div></div>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Service-area demand</h2><p className="subtle">Pincode workload and shipment-size mix using the editable Capacity Master rule.</p></div><span className="status-pill neutral">{pincodes.length} pincodes</span></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Pincode</th><th>Delivered</th><th>Share</th><th>Active IDs</th><th>Volumetric</th><th>Small</th><th>Avg kg</th><th>Dimension coverage</th></tr></thead><tbody>{pincodes.slice(0, 25).map((row) => <tr key={row.postal_code}><td><strong>{row.postal_code}</strong></td><td>{fmt(num(row.delivered))}</td><td>{pincodeDelivered ? `${fmt(num(row.delivered) / pincodeDelivered * 100, 1)}%` : "—"}</td><td>{fmt(num(row.active_ids))}</td><td>{fmt(num(row.volumetric))}<small>{num(row.delivered) ? `${fmt(num(row.volumetric) / num(row.delivered) * 100, 1)}%` : "—"}</small></td><td>{fmt(num(row.small))}<small>{num(row.delivered) ? `${fmt(num(row.small) / num(row.delivered) * 100, 1)}%` : "—"}</small></td><td>{row.average_weight_kg == null ? "—" : fmt(num(row.average_weight_kg), 2)}</td><td>{num(row.delivered) ? `${fmt(num(row.dimension_ready) / num(row.delivered) * 100, 1)}%` : "—"}</td></tr>)}{!pincodes.length ? <tr><td className="empty-cell" colSpan={8}>No pincode-level shipment facts are available for this range.</td></tr> : null}</tbody></table></div></section>
  </div></AppShell>;
}
