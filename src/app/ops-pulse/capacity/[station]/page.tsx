...
export default async function CapacityStationPage({ params, searchParams }: { params: { station: string }; searchParams?: SearchParams }) {
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="panel capacity-area-pay"><div className="panel-head"><div><h2>Service-area map & hiring pay</h2><p className="subtle">Internal route geometry, assigned bike/van DAs, pincodes and approved hiring rates.</p></div><a className="button secondary compact" href="/master/capacity">Manage routes</a></div>
      <CapacityServiceMap routes={serviceRouteResult.rows} station={latitude && longitude ? { lat: latitude, lng: longitude, label: stationCode } : null}/>
      {capacityMap ? <div className="capacity-map-meta"><strong>Reference source: {capacityMap.name}</strong><span>The internal map above is operational master data; Google My Maps is retained only for comparison.</span><a href={capacityMap.mapUrl} target="_blank" rel="noreferrer">Open reference map</a></div> : null}
