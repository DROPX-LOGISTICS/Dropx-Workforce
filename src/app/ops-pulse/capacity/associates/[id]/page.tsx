...
export default async function AssociateCapacityPage({ params, searchParams }: { params: { id: string }; searchParams?: SearchParams }) {
  ...
  const rows = (shipmentResult.data ?? []).filter((row) => row.associate_id === id);
  ...
  const name = rows.find((row) => row.associate_name)?.associate_name || id;
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <div className="capacity-station-toolbar"><a className="button secondary compact" href={`/ops-pulse/capacity/associates?station=${station}&from=${start}&to=${end}`}>← Associate SPR</a><form method="get"><input type="hidden" name="station" value={station}/><label>From<input type="date" name="from" defaultValue={start}/></label><label>To<input type="date" name="to" defaultValue={end}/></label><button className="button compact">Apply</button></form></div>
    <section className="performance-summary-grid"><article><span>Days worked</span><strong>{daily.length}</strong><small>Shipment-active days</small></article><article><span>Total workload</span><strong>{fmt(total)}</strong><small>Amazon + SWA + C-return</small></article><article><span>Average allocation</span><strong>{fmt(average, 1)}</strong><small>Workload per active day</small></article><article><span>High-load days</span><strong>{highDays}</strong><small>Above safe SPR {fmt(safe)}</small></article></section>
