...
export default async function ServiceNetworkPage({ searchParams }: { searchParams?: Search }) {
  ...
  return <AppShell active="Service Network" pageCode="service_network"><div className="ops-command-center capacity-workspace">
    ...
    {!selected ? <section className="panel"><div className="empty-cell">No Amazon EDSP or Flipkart ODH/MDH station is available in your jurisdiction.</div></section> : <>
      ...
      <ServiceNetworkMap station={stationLat && stationLng ? { code: selected.station_code, lat: stationLat, lng: stationLng } : null} features={mapFeatures} metrics={metrics} radiusKm={rule?.serviceRadiusKm ?? null}/>
