...
type Search = { station?: string; client?: string; from?: string; to?: string };
...
export default async function ServiceNetworkPage({ searchParams }: { searchParams?: Search }) {
  ...
  const from = validDate(searchParams?.from) ? String(searchParams?.from) : `${to.slice(0, 8)}01`;
  ...
  const approved = new Set([...(rule?.pincodeOwnership ?? []), ...layer.features.filter(feature => /^\d{6}$/.test(feature.name)).map(feature => feature.name)]);
  ...
  const error = locationResult.error || rulesResult.error || mapsResult.error || pincodeResult.error?.message || stationResult.error?.message;

  return <AppShell active="Service Network" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    <PageHead eyebrow="Network Planning" title="Service Network" subtitle="Jurisdiction, approved service boundaries, pincode demand and bike/van capacity in one operational map." action={<a className="button secondary compact" href="/ops-pulse/master/service-network">Service Network Master</a>} />
    ...
    {!selected ? <section className="panel"><div className="empty-cell">No Amazon EDSP or Flipkart ODH/MDH station is available in your jurisdiction.</div></section> : <>
      <div className="capacity-action-line"><strong>Jurisdiction</strong><span>{[selected.region, selected.aom, selected.cluster, selected.station_code].filter(Boolean).join(" › ") || selected.station_code} · {clientName(selected)} · {providerName(selected)}</span></div>
      ...
      <ServiceNetworkMap station={stationLat && stationLng ? { code: selected.station_code, lat: stationLat, lng: stationLng } : null} features={layer.features} metrics={metrics} radiusKm={rule?.serviceRadiusKm ?? null}/>
      ...
      <section className="panel"><div className="panel-head"><div><h2>Pincode capacity plan</h2><p className="subtle">Demand-backed requirement for the selected period. Actual bike/van availability remains unmapped until DA designation is maintained.</p></div><span className="status-pill neutral">{from.split("-").reverse().join("/")} – {to.split("-").reverse().join("/")}</span></div><div className="table-wrap"><table className="capacity-daily-table"><thead><tr><th>Pincode</th><th>Delivered</th><th>Avg/day</th><th>Share</th><th>Active IDs</th><th>Volumetric</th><th>Small</th><th>Bike required</th><th>Van required</th><th>Signal</th></tr></thead><tbody>{metrics.sort((a,b) => b.delivered-a.delivered).map(row => { const conflict = approved.size > 0 && !approved.has(row.pincode); const gap = row.totalRequired == null ? null : row.totalRequired - row.activeIds; return <tr key={row.pincode}><td><strong>{row.pincode}</strong>{conflict ? <small className="metric-bad-text">Outside jurisdiction</small> : null}</td><td><a href={`/ops-pulse/capacity/shipments?station=${selected.station_code}&pincode=${row.pincode}&from=${from}&to=${to}`}>{fmt(row.delivered)}</a></td><td>{fmt(row.delivered / Math.max(1,row.activeDays),1)}</td><td>{delivered ? `${fmt(row.delivered/delivered*100,1)}%` : "—"}</td><td>{row.activeIds}</td><td>{fmt(row.volumetric)} · {row.delivered ? fmt(row.volumetric/row.delivered*100,1) : 0}%</td><td>{fmt(row.small)} · {row.delivered ? fmt(row.small/row.delivered*100,1) : 0}%</td><td>{row.bike ?? "Configure"}</td><td>{row.van ?? "Configure"}</td><td><span className={`status-pill ${conflict || (gap != null && gap > 0) ? "bad" : gap == null ? "neutral" : "good"}`}>{conflict ? "Boundary conflict" : gap == null ? "Rule missing" : gap > 0 ? `Need ${gap}` : "Covered"}</span></td></tr>; })}{!metrics.length ? <tr><td className="empty-cell" colSpan={10}>No pincode shipment facts are available for this station and period.</td></tr> : null}</tbody></table></div></section>
