...
type ExceptionRow = { profileType: string; profileId: string; table: string; name: string; designation: string; location: string; category: string; ruleCode: string; issue: string; detail: string; sourceUpdatedAt: string; profile: ProfileRow };
...
async function loadExceptions(companyId: string, authorization: AuthorizationContext) {
  ...
  const profileResults = await Promise.all(SOURCES.map(async (source) => {
    const select = source.employee ? `${BASE_FIELDS}, ifsc, designations (name)` : `${BASE_FIELDS}, ifsc_code, designation`;
    let query = supabaseAdmin!.from(source.table).select(select).eq("company_id", companyId).eq("is_active", true);
  ...
  const add = (source: typeof SOURCES[number], profile: ProfileRow, ruleCode: string, issue: string, detail: string, sourceUpdatedAt?: string | null) => {
    ...
    rows.push({ profileType: source.profileType, profileId: profile.id, table: source.table, name: clean(profile.full_name) || "Unnamed profile", designation: clean(source.employee ? designationRelation?.name : profile.designation) || "-", location: clean(station?.station_code) || "-", category: source.category, ruleCode, issue, detail, sourceUpdatedAt: sourceUpdatedAt || profile.updated_at || new Date(0).toISOString(), profile });
  };
...
export default async function PeopleExceptionsPage({ searchParams }: { searchParams?: { error?: string; notice?: string; page?: string; edit?: string } }) {
  ...
  return <AppShell active="Exception" pageCode="people_exceptions">
    ...
    <section className="panel"><div className="panel-head"><div><h2>Open exceptions</h2><p className="subtle">Active profiles only. Partial matches remain in Under Review.</p></div></div><div className="table-wrap"><table><thead><tr><th>Person</th><th>Category</th><th>Designation</th><th>Location</th><th>Exception</th><th>Details</th><th>Action</th></tr></thead><tbody>
      {visibleRows.length ? visibleRows.map((row) => <tr key={key(row.profileType, row.profileId, row.ruleCode)}><td><strong>{row.name}</strong></td><td>{row.category}</td><td>{row.designation}</td><td>{row.location}</td><td><StatusPill status={row.issue} /></td><td>{row.detail}</td><td><PendingLink className="button secondary compact" href={`/people/exceptions?page=${page}&edit=${encodeURIComponent(key(row.profileType, row.profileId, row.ruleCode))}`} scroll={false}>Clear</PendingLink></td></tr>) : <tr><td className="empty-cell" colSpan={7}>No open people exceptions.</td></tr>}
