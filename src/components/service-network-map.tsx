...
export function ServiceNetworkMap({ station, features, metrics, radiusKm }: {
  ...
  const [layer, setLayer] = useState<"volume" | "mix" | "capacity">("volume");
  ...
  const color = (metric?: Metric) => !metric ? "#64748b" : layer === "mix" ? (metric.volumetric / Math.max(1, metric.delivered) >= .25 ? "#dc2626" : "#16a34a") : layer === "capacity" ? ((metric.bike ?? 0) + (metric.van ?? 0) > metric.activeIds ? "#dc2626" : "#2563eb") : "#ea580c";

  return <div className={styles.shell}>
    <div className={styles.toolbar}><div><strong>Jurisdiction & demand map</strong><span>{pincodeFeatures.length} plotted pincodes · {metrics.length} with shipment evidence</span></div><div className={styles.controls}><select value={layer} onChange={event => setLayer(event.target.value as typeof layer)}><option value="volume">Volume</option><option value="mix">Shipment mix</option><option value="capacity">Capacity gap</option></select><select value={selected} onChange={event => setSelected(event.target.value)}><option value="all">All pincodes</option>{pincodeFeatures.map(feature => <option key={feature.name}>{feature.name}</option>)}</select><button type="button" onClick={() => setZoomOffset(v => Math.min(4, v + 1))}>+</button><button type="button" onClick={() => setZoomOffset(v => Math.max(-4, v - 1))}>−</button></div></div>
    ...
    </svg>{!features.length ? <div className={styles.empty}>No approved station boundary is available. Add or sync its KML layer in Service Network Master.</div> : null}<small>Approved service boundary · basemap © OpenStreetMap contributors</small></div>
  </div>;
}
