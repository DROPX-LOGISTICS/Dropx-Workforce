...
import {registerStageHref} from '@/lib/workforce-register-views';
...
export function WorkforceJourneySummary({overview}:{overview:ReturnType<typeof workforceOverview>|null}){
 return <section className="wf-command-panel" aria-label="Associate lifecycle overview">
  ...
  {overview?<div className={styles.grid}>{Object.entries(joiningStages).map(([stage,label])=><PendingLink className={styles.stage} key={stage} href={registerStageHref(stage)}><span><strong>{label}</strong><small>{hints[stage as JoiningStage]}</small></span><b>{overview.counts[stage as JoiningStage]}</b></PendingLink>)}</div>:<p className={styles.notice}>Lifecycle counts are unavailable. <PendingLink href="/delivery-network/associates">Open associates</PendingLink></p>}
