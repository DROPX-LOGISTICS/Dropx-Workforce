...
export const accessPages = [
  ...
  { code: "service_network", name: "Service Network", sort_order: 84.1 },
  { code: "service_network_master", name: "Service Network Master", sort_order: 84.2 },
...
async function upsertPermissionRows(supabase: SupabaseClient, companyId: string, rows: PermissionRow[]) {
  ...
  const { data: existing, error: existingError } = await supabase
    ...
    .select("role_id, page_id")
    .eq("company_id", companyId)
    .in("role_id", roleIds)
    .in("page_id", pageIds);
  if (existingError) throw new Error(existingError.message);

  const existingKeys = new Set((existing ?? []).map((row) => `${row.role_id}:${row.page_id}`));
  const insertRows = rows
    .filter((row) => !existingKeys.has(`${row.role_id}:${row.page_id}`))
    .map((row) => ({ company_id: companyId, ...row }));
  const updateRows = rows.filter((row) => existingKeys.has(`${row.role_id}:${row.page_id}`));
...
export async function ensureAccessPages(supabase: SupabaseClient, companyId: string) {
  ...
  for (const page of accessPages) {
    ...
    if (existing) {
      ...
      const { data, error } = await supabase
        .from("app_pages")
        .insert({ company_id: companyId, ...page, is_active: true, updated_at: now })
        .select("id, code, is_active")
        .single();
      if (error) {
