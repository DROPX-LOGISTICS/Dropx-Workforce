...
const sls = [
  ["gst_pendency", "%GST Pendency", 9, .001, "lower", 2.5, "percent"],
  ["helmet_adherence", "Helmet Adherence", 2, .985, "higher", 10, "percent"],
  ["dot_standard", "DOT – Standard", 4, .935, "higher", 5, "percent"],
  ["dot_premium", "DOT – Premium", 3, .955, "higher", 5, "percent"],
  ["dds_standard", "DDS – Standard", 6, .89, "higher", 5, "percent"],
  ["dds_premium", "DDS – Premium", 5, .94, "higher", 5, "percent"],
  ["c_ret_fdps", "C-Ret FDPS", null, .955, "higher", 5, "percent"],
  ["in_facility_losses", "In-Facility Losses vs Goal", 7, 1, "lower", 5, "ratio"],
  ["short_cash", "Short Cash", 8, .001, "lower", 5, "percent"],
  ["open_cod", "Open COD (>7 Days)", 10, .001, "lower", 5, "percent"],
  ["non_delivered_good_scan", "Non-Delivered Good Scan", 11, .9, "higher", 5, "percent"],
  ["unsuccessful_pickup_good_scan", "Unsuccessful Pickup Good Scan", 12, .83, "higher", 5, "percent"],
  ["rts_dpmo", "RTS DPMO", null, 500, "lower", 5, "dpmo"],
  ["undel_dpmo", "Undel DPMO vs Goal", null, 1, "lower", 5, "ratio"],
  ["swa_cod_dsr", "SWA COD DSR", 13, .684, "higher", 10, "percent"],
  ["swa_prepaid_dsr", "SWA Prepaid DSR", 14, .98, "higher", 2.5, "percent"],
  ["forward_cps", "Forward-Leg Contacts / Shipment", 15, .003, "lower", 5, "percent"],
  ["reverse_cps", "Reverse-Leg Contacts / Shipment", 16, .0105, "lower", 2.5, "percent"],
  ["dnr_dpmo", "DNR DPMO Within 48 Hours", null, 900, "lower", 2.5, "dpmo"],
  ["dnr_rescue", "DNR Rescue Rate", 18, .85, "higher", 2.5, "percent"],
  ["readme_otr", "ReadMe OTR", 19, .95, "higher", 2.5, "percent"]
] as const;
...
export const performanceTargetSeeds: PerformanceTarget[] = [
  ...
  ...sls.map((row, index) => ({ metricKey: row[0], label: row[1], short: row[1], reportType: "sls" as const, sourceIndex: row[2], target: row[3], direction: row[4], weight: row[5], unit: row[6], displayOrder: index + 1, isActive: true }))
...
export async function loadPerformanceTargets(companyId: string) {
  ...
  let result = await supabaseAdmin.from("report_import_master").select("id,description").eq("company_id", companyId).eq("parser_type", "performance_target").order("source_code");
  ...
  return { rows: (result.data ?? []).map(parse).filter(Boolean) as PerformanceTarget[], error: result.error?.message ?? null };
}
