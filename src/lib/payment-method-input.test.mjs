...
function form() { const f = new FormData(); f.set('code', ' per_packet '); f.set('name', ' Per Packet '); f.append('field_ids', id); return f; }
test('normalizes codes and deduplicates shared field IDs', () => {
  const f = form(); f.append('field_ids', id);
  assert.deepEqual(parsePaymentMethodInput(f, false), { id: null, code: 'PER_PACKET', name: 'Per Packet', fieldIds: [id] });
});
