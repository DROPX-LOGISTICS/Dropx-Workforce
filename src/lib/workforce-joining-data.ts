...
export type JoiningProfile = JoiningPerson & {full_name: string; dropx_id: string | null; biometric_id: string | null; designation: string | null; onboarding_application_source: string | null};
...
export async function loadWorkforceJoining(authorization: AuthorizationContext, options: {from?: string; to: string; evidence?: boolean}) {
  ...
  let profilesQuery = db.from("workforce").select("id,full_name,dropx_id,biometric_id,designation,location_id,source_profile_type,source_profile_id,onboarding_status,lifecycle_status,is_active,onboarding_approved_at,last_working_date,onboarding_application_source")
  ...
  const [peopleResult, plansResult, mappingsResult, stationsResult, designationResult, policyResult] = await Promise.all([
    ...
    readAllRows(db.from("designations").select("code,name,designation_category:designation_categories!designations_designation_category_id_fkey(people_module)").eq("company_id",company).order("id")),
  ...
  const error = peopleResult.error || mappingsResult.error || stationsResult.error || designationResult.error || policyResult.error;
  if (error) throw new Error(error.message);
  if (plansResult.error) throw new Error(`Joining database release is not ready: ${plansResult.error.message}`);
  const roles = new Set((designationResult.data ?? []).filter(row => firstDesignationBusinessCategory(row.designation_category)?.people_module === "delivery_network").flatMap(row => [row.code,row.name].map(value => String(value).trim().toLowerCase())));
  const profiles = ((peopleResult.data ?? []) as JoiningProfile[]).filter(row => roles.has(String(row.designation).trim().toLowerCase()));
