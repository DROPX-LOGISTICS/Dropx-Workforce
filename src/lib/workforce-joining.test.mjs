...
test("training reaches payroll earnings independently of provider activation",()=>{
  const result=calculateWorkforceEarnings({from:"2026-08-01",to:"2026-08-31",shipments:[],mappings:[],workforce:[person],providers:[],stations:[{id:"station",station_code:"TEST"}],rateCards:[],campaigns:[],adjustments:[],joiningPlans:[plan],trainingAttendance:[day(),day("2026-08-02")]});
  assert.equal(result.totalNet,800);assert.equal(result.readyWorkers,1);assert.equal(result.summaries[0].workDays,2);
  assert.equal(result.lines[0].sourceType,"training");assert.equal(result.lines[0].trace.joining_plan_version,1);
  assert.equal(result.totalShipments,0);
});
test("missing bank details holds otherwise eligible training earnings",()=>{
  const result=calculateWorkforceEarnings({from:"2026-08-01",to:"2026-08-31",shipments:[],mappings:[],workforce:[{...person,bank_account_no:null}],providers:[],stations:[],rateCards:[],campaigns:[],adjustments:[],joiningPlans:[plan],trainingAttendance:[day()]});
  assert.equal(result.totalNet,400);assert.equal(result.heldWorkers,1);assert.match(result.summaries[0].holdReasons.join(),/Bank/);
});
