...
-- DropX One is the individual mobile identity and is not an administrative web
-- portal. A People designation may therefore have no web-portal eligibility.
-- Existing arrays and memberships are intentionally left untouched.
...
-- Whether somebody belongs in People is a legal/master-data classification,
-- not a statement that they can sign in to the People administration portal.
-- Keep Employee/Independent Contractor compatibility projections aligned even
-- when the designation is DropX One-only.
...
comment on column public.designations.portal_scopes is
  'Administrative web portals eligible for this designation. Empty means DropX One-only; DropX One mobile access is governed by app_page_access.';
